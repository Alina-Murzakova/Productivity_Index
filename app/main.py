...
from gtm import gtm_date_convert
from Arps import calc_arps
from Charts_xlwings import charts
from time import sleep

# data_file = r"C:\Users\Alina\Desktop\Python\Kprod\input\Крайнее.xlsx"  # Файл для расчета
# data_file_gtm = r"C:\Users\Alina\Desktop\Python\Kprod\input\Крайнее_ГТМ.xls"  # Файл новая стратегия
...
def PI(df_initial, df_gtm, data_file, max_distance, Flag_smooth, win, pb, label_5):
    ...
    # Список скважин, соответствующий характеру работы на последнюю дату
    ...
    for prod_well in wells_prod:
        ...
        try:
            ...
            error = "проблемы со сходимостью"
            dict_errors = {'Доб.скважина': prod_well, 'Нагн.скважина': 0, 'Причина': error}
            df_errors = df_errors._append(dict_errors, ignore_index=True)
            continue

        data_prod_with_arps = data_prod_with_arps[[date, 'hyperbolic', 'hyperbolic_power', 'double']]  # 'exponential', 'harmonic', 'minimize'
        ...
        data_prod.replace([np.nan, -np.nan], 0, inplace=True)
    ...
    # Формируем новый путь для сохранения в output/
    dir_output = os.path.join(data_path, 'output')
    os.makedirs(dir_output, exist_ok=True)  # создаем папку, если нет

    new_wb.save(str(os.path.join(dir_output, filename)).replace(".xlsx", "") + "_out.xlsx")
    # app1.kill()
    app1.quit()
    del app1
